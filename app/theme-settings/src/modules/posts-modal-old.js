...
export default function getPostsModal(obj) {
	...
	function Init() {
		...
		const load_more_btn_template = '<button type="button" class="wdst-button load-more">Fetch next page</button>';
		...
		if (window[`${obj.fetch_action}-setup`] === true) {
			notification.prompt('Enter max posts per fetch count. The minimum is 30: ', openModal);
		...
		function checkNoResults(results = null) {
			...
			if (!results) {
				let not_found_msg = modal.querySelector('.wdst-modal-not-found-msg');
		...
		function openModal(set_max_posts_per_fetch) {
			...
			console.log(set_max_posts_per_fetch);
			...
			if (Number(total_posts) > Number(max_posts_per_fetch)) {
				is_lite_mode = true;
				if (modal_title) {
		...
		function fetchMorePostsHandler() {
			...
			try {
				fetch(window.location.origin + `/wp-json/wp/v2/posts/?per_page=30&page=${window.next_fetched_page}`)
					.then(response => {
					...
					.then(data => {
						...
						jQuery.post( ajaxurl, data_obj, 'json').
							...
							if (response) {
								...
								if (!document.querySelector('.wdst-modal-not-found-msg')) {
		...
		function modalHandler($content) {
			...
			if (is_lite_mode && !modal.querySelector('.wdst-button.load-more')) {
				get_posts_btn.insertAdjacentHTML('beforebegin', load_more_btn_template);
				load_more_btn = modal.querySelector('.wdst-button.load-more');
				load_more_btn.classList.add('inactive');
			}
